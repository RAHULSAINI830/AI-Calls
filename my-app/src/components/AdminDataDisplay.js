// src/components/AdminDataDisplay.js
    ...
    const fetchCalls = async () => {
      try {
        ...
        if (
          ...
          const filteredCalls = response.data.response.calls.filter(call => call.model_id === modelId);
    ...
    <div>
      ...
      {calls.length ? (
        <ul>
          {calls.map(call => (
            <li key={call.call_id}>
              Caller: {call.phone_number_from} | Time: {new Date(call.start_time).toLocaleString()}
            </li>
          ))}
        </ul>
      ) : (
